...
def isInBorders( x : tuple[float],
                    lBorder : tuple[float],
                    rBorder : tuple[float]) -> bool:
    
    pointToBordersDistance = sum([abs(rBorder[i] - x[i]) + abs(lBorder[i] - x[i]) for i in range(len(x))])
    bordersDistance = sum([abs(rBorder[i] - lBorder[i]) for i in range(len(rBorder))])
...
def getPointNear(
                cur: tuple[float],
                argIncrement: float,
                argNumber: int) -> tuple[int]:
    newArgs = list(cur)
    newArgs[argNumber] += argIncrement
    return tuple(newArgs)

def getArgValPairNear(function: Callable[[tuple[float]], float],
                      cur: ArgValPair,
                      argIncrement: float,
                      argNumber: int) -> ArgValPair:
    newArgs = getPointNear(cur.args, argIncrement, argNumber)
    return ArgValPair(newArgs, function(newArgs))


def vectorTriadMethod(  f: Callable[[tuple[float]], float],
                        a: tuple[float],
                        b: tuple[float],
